...
def _update_room(code: str, transform: Callable[[Room], Room]) -> Room:
    try:
        return store.update(code.strip().upper(), transform)
    except KeyError as error:
        raise HTTPException(404, "room not found") from error
...
def join_room(code: str):
    try:
        updated = _update_room(code, rooms.join_room)
...
def get_state(code: str, token: str):
    try:
        def touch(room: Room) -> Room:
            return rooms.touch_presence(room, rooms.seat_for_token(room, token))

        updated = _update_room(code, touch)
        seat = rooms.seat_for_token(updated, token)
    except rooms.RoomError as error:
        raise _room_error(error) from error
...
def submit_move(code: str, payload: MoveRequest):
    try:
        def move(room: Room) -> Room:
            seat = rooms.seat_for_token(room, payload.token)
            return rooms.apply_move(room, seat, payload.move, payload.expected_move_count)

        updated = _update_room(code, move)
        seat = rooms.seat_for_token(updated, payload.token)
    except rooms.RoomError as error:
        raise _room_error(error) from error
    return rooms.snapshot(updated, seat)
